...
const applyTranslationsModals = () => {
    const map = [
        ...
        ['#checkout-modal .checkout-left-col .checkout-section:nth-child(1) h3', 'shippingAddress'], ['#checkout-modal .checkout-left-col .checkout-section:nth-child(2) h3', 'shippingMethod'], ['#checkout-modal .summary-section h3', 'orderSummary'],
    ...
    auth.querySelector('#signup-username').placeholder = t('usernamePlaceholder');
    auth.querySelector('#auth-confirm-password').placeholder = t('confirmPasswordPlaceholder');

    const cf = document.getElementById('checkout-form');
    const placeholderKeys = ['fullName', 'email', 'phone', 'address', 'city', 'stateProvince', 'zipCode'];
    [...cf.querySelectorAll('input:not([type="radio"])')].forEach((inp, i) => {
        if (placeholderKeys[i]) inp.placeholder = t(placeholderKeys[i]) || inp.placeholder;
    });
