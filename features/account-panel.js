...
import { openOrderTracking } from './order-tracking-panel.js';
...
export const renderOrderHistory = () => {
    ...
    history.slice().reverse().forEach(order => {
        ...
        const isCancellable = currentStatus !== 'delivered' && currentStatus !== 'cancelled';
        
        const statusBadgeClass = `status-badge status-${currentStatus}`;
        const statusLabels = {
            ...
            <div class="account-order-card">
                ...
                <div class="order-footer">
                    ...
                    <button class="track-order-btn" data-order-id="${order.id}">
                        ...
                        <span>${isCancellable ? t('trackAndManage') || 'Track & Manage' : t('viewDetails') || 'View Details'}</span>
                    </button>
                </div>
            </div>
        `);
...
export const renderAvailableCoupons = () => {
    ...
    coupons.forEach(coupon => {
        ...
        const trKey = `discount${coupon.code}`;
        const translated = t(trKey);
        // If translation engine returns the raw key, fallback to the config description
        const desc = translated === trKey ? coupon.description : translated;
...
export const renderPointsBadge = (user) => {
    ...
    const existingWidget = profileCard.querySelector('.aether-points-widget');
    if (existingWidget) existingWidget.remove();

    const pointsValue = user.aetherPoints;
...
export const initAccountPanel = () => {
    ...
    // Welcome coupon notification on new registration
    document.addEventListener('app:registerSuccess', () => {
        setTimeout(() => {
            NotificationSystem.showToast(
                '🎉 Welcome! You\'ve received WELCOME10 (10% off) and FREESHIP coupons — check your Coupons section!',
                'success'
            );
        }, 800);
    });
