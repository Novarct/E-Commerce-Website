...
const buildHeaderMap = (headers) => {
    ...
    const SILENT_SINK_INDEX = 999;
    
    for (const [canonical, aliases] of Object.entries(keyAliases)) {
        map[canonical] = SILENT_SINK_INDEX; // Naked Logic: Map missing columns to a guaranteed empty string index
...
const parseCSVProducts = (csvText) => {
    ...
    return lines.slice(1)
        ...
        .map(line => {
            ...
            values[999] = ''; // SILENT_SINK_INDEX 

            // Naked Logic: Ensure the payload shape guarantees a string at every mapped index
